...
class AMPOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        amp_dataset_dir = resolve_project_path(train_cfg["amp_motion_files"])
        validate_amp_dataset_dir(amp_dataset_dir, train_cfg["amp_num_frames"])
        self.amp_data = G1_AMPLoader(
            device,
            time_between_frames=1/50.0,
            preload_transitions=True,
            num_preload_transitions=train_cfg["amp_num_preload_transitions"],
            motion_files=str(amp_dataset_dir),
