...
class ReplayBufferMulti:
    ...
    def __init__(self, obs_dim, buffer_size, num_amp_frames, device):
        ...
        self.masks = torch.zeros(buffer_size, dtype=torch.bool).to(device)
    ...
    def insert(self, states, masks=None):
        """Add new states to memory."""
        num_states = states.shape[0]
        start_idx = self.step
        end_idx = self.step + num_states
        if masks is None:
            masks = torch.ones(num_states, dtype=torch.bool, device=self.device)
        if end_idx > self.buffer_size:
            ...
            self.masks[self.step:self.buffer_size] = masks[:self.buffer_size - self.step]
            self.masks[:end_idx - self.buffer_size] = masks[self.buffer_size - self.step:]
        else:
            self.states[start_idx:end_idx] = states
            self.masks[start_idx:end_idx] = masks
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        for _ in range(num_mini_batch):
            ...
            yield (
                ...
                self.masks[sample_idxs].to(self.device),
            )
