...
class AMP_PPO:
    ...
    def process_env_step(self, obs, rewards, dones, extras, amp_obs, amp_obs_frames=None, amp_mask=None):
        ...
        if amp_mask is None:
            amp_mask = torch.ones(obs["policy"].shape[0], dtype=torch.bool, device=self.device)
        if amp_obs_frames is not None:
            self.amp_storage.insert(amp_obs_frames, amp_mask)
        else:
            self.amp_storage.insert(self.amp_transition.observations, amp_mask)
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            policy_states, policy_amp_mask = sample_amp_policy
            ...
            mask_push = policy_amp_mask.bool()
